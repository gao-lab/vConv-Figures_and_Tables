...
def main():
    ...
    KernelMotifPath = "../ModelAUC/JasperMotif/motif/"
    dataPathlist = glob.glob(KernelMotifPath+"/*")
    for datapath in dataPathlist:
        GenerateMemeFormat(datapath+"/recover_PWM/", datapath+"/memeformatall.txt")

    KernelMotifPath = "../ModelAUC/JasperMotif/motifCNN/"
    dataPathlist = glob.glob(KernelMotifPath+"/*")
    for datapath in dataPathlist:
        GenerateMemeFormat(datapath+"/recover_PWM/", datapath+"/memeformatall.txt")
