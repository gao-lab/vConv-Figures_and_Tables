...
if __name__ == '__main__':
	...
	# AUC, AUCAixsDB, WorseKeyDB, DeepBindlist = CompareModels(deepbindResult, DatatypeAUC,"deepbind")
	#
	...
	# 				 ["vConv-based "+"\n"+"network","DeepBind network"],statisticRoot+"/Pic","DeepBind network")


	##Draw DeepBind worse Data comparision
	# keylists = DeepBindWorseDataSelect(deepbindResult, DatatypeAUC)

	# DrawCompareResult(keylists, DatatypeAUC, CNNResult,
	# 				 ["vConv","CNN1layers128motifs"],statisticRoot+"/Pic/worseDeepBind/","convolution-based network from Zeng et al., 2016")

	# Worse Analysis

	# WorseKeyCNN = get_Worsedata_info()
	# DrawCompareResult(WorseKeyCNN, DatatypeAUC, CNNResult,
	# 				 ["vConv", "CNN1layers128motifs"],statisticRoot+"/Pic/WorseCNN/", "convolution-based network from Zeng et al., 2016")
	...
	#
	# WriteWorseKey("../../train/ChIpSeqworseData/WorseKey.txt", Worselist)
	WriteWorseKey(statisticRoot+"/WorseKeyCNN.txt", WorseKeyCNN)
	...
	StaWorseKey(WorseKeyCNN, AUCAixsCNN, dataNumdict, dataNumlist, statisticRoot+"/Pic/")
	SmallSizeDataAnalyse(dataNumdict, DatatypeAUC, CNNResult, statisticRoot+"/Pic/")

	AUCpath = statisticRoot + "/AUC/"
	mkdir(AUCpath)
	np.savetxt(AUCpath + "Zeng.txt", np.asarray(Zeng["convolution-based network" + "\n" + "from Zeng et al., 2016"]))
	np.savetxt(AUCpath + "vConv.txt", np.asarray(Zeng["vConv-based " + "\n" + "network"]))
