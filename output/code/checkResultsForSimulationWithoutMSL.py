...
def DrawBox(dataName, path, outputName):
    ...
    for i in range(len(dataName)):
        ...
        pvalue = stats.levene(Pddict["vConv-based \n network"], Pddict["No SHL \n vConv-based \n network"])[1]
        StdComDict[outputtem] = [round(pvalue, 3), np.std(Pddict["vConv-based \n network"]),
                                np.std(Pddict["No SHL \n vConv-based \n network"])]
        print(StdComDict)
    StdComDF = pd.DataFrame(StdComDict)
    StdComDF.index = ["p-value", "Std. for networks with MSL", "Std. for networks without MSL"]
    StdComDF.to_csv("../../vConvFigmain/supptable23/SuppTable3.csv")
...
def DrawErrorBar(dataName, data, path,OutputName, pvaluedict):
    ...
    CNNresult = data["vCNNNSHL"]
    # sns.set_style("darkgrid")
    difference = {}
    Std = {"Name":[],"vCNN":[],"vCNNNSHL":[]}
    ...
    for i in range(len(dataName)):
        ...
        Std["vCNNNSHL"].append(np.std(CNNresult[key]))
...
def Forscatter(dataName, data, path, OutputName):
    ...
    for i in range(len(dataName)):
        ...
        Barplot = plt.scatter(vCNNresult[key],CNNresult[key],s=1)
        numbers = np.where(np.asarray(vCNNresult[key]) - np.asarray(CNNresult[key])>0)[0].shape[0]
        numbers = float(numbers)/len(vCNNresult[key])

        plt.title("AUROC comparision on "+dataName[i]+": "+ "%.2f%%" % (numbers * 100), font2)
        plt.xlabel("vConv-based network AUROC")
        plt.ylabel("vConv-based network withou SHL AUROC")
...
if __name__ == '__main__':
    ...
    for item in model_lst:
        ...
        mkdir("../../output/ModelAUC/JasperMotif/SHLcompare/")
        Df.to_csv("../../output/ModelAUC/JasperMotif/SHLcompare/"+ item + "AUC.csv")
        for key in aucouttem.keys():
            print(key)
            np.savetxt("../../output/ModelAUC/JasperMotif/SHLcompare/" + key +"_" + item + "_auc.txt", np.asarray(aucouttem[key]))
    ...
    mkdir("../../output/ModelAUC/JasperMotif/SHLcompare/pic/")
    pvaluedict = DrawBox(dataName, "../../output/ModelAUC/JasperMotif/SHLcompare/",OutputName)

    DrawErrorBar(dataName, AUCDifference, "../../output/ModelAUC/JasperMotif/SHLcompare/pic/",OutputName,pvaluedict)

    # Forscatter(dataName, AUCDifference, "../../output/ModelAUC/JasperMotif/SHLcompare/pic/",OutputName)
    # Forscatter(dataName, AUCCom, "../../output/ModelAUC/JasperMotif/pic/SHLcompare/Random",OutputName)
