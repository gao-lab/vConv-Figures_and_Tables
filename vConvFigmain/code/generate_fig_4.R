...
accuracy.increment.ggplot <- ggboxplot(data=accuracy.increment.dt, x="tool.to.plot", y="accuracy.increment", fill="tool.to.plot", palette="npg", bxp.errorbar=TRUE) + labs(x="", y="Improvement of accuracy\nby vConv-based motif discovery", fill="") + scale_fill_manual(values=c("#395486", "#009F86", "#2FB9D3")) + scale_y_continuous(breaks=seq(-0.06, 0.12, 0.02)) + theme(axis.text.x=element_blank()); accuracy.increment.ggplot
