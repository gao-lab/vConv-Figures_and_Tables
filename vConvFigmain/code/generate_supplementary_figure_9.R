...
auroc.dt <- foreach(temp.dataset=c("2", "4", "6", "8", "TwoDif1", "TwoDif2", "TwoDif3")) %do% {
    data.table(
        ...
        vConv.AUROC=paste(sep="", "./vConvFigmain/files.SF9/JasperMotif/SHLcompare/", temp.dataset, "_vCNN_auc.txt") %>% {scan(file=., what=double())},
        vConvNoSHL.AUROC=paste(sep="", "./vConvFigmain/files.SF9/JasperMotif/SHLcompare/", temp.dataset, "_vCNNNSHL_auc.txt") %>% {scan(file=., what=double())}
...
temp.ggplot <- ggbarplot(data=auroc.dt, x="dataset.to.plot", y="AUROC.increment", fill="#53C1A5", palette="npg", add="mean_se") + labs(x="", y="Improvement in AUROC\nby enabling Shannon loss\nfor vConv-based network") + theme(axis.text.x=element_text(angle=45, hjust=1)); temp.ggplot %>% {ggsave(filename="./vConvFigmain/result/Supplementary.Fig.9/Supplementary.Fig.9.png", plot=., device="png", width=14, height=12, units="cm")}
