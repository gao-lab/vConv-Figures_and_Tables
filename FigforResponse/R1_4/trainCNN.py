...
def run_Simulation_data(KernelLen, KernelNum, RandomSeed,opt):
    def get_data_info_list(root_dir = ""):
        ...
        return ret
    cmd = "python ../R_2_b/code/main.py"
    mode_lst = ["CNN"]

    data_root = "../../data/JasperMotif/HDF5/"
    result_root = "../../output/result/Speed/SpeedOneachEpoch"
    ...
    for data_info in data_info_lst:
        for mode in mode_lst:
            ...
            modelsave_output_prefix = result_path + '/CNN/'+opt+"/"
            modelsave_output_filename = modelsave_output_prefix + "/model_KernelNum-" + str(
                KernelNum) + "_KernelLen-" + str(KernelLen) + "_seed-" + str(RandomSeed) + ".hdf5"
...
if __name__ == '__main__':
    ...
    rho = 0.9
    epsilon = 1e-8
    run_Simulation_data(str(KernelLen), str(KernelNum), str(RandomSeed),"adadelta")
