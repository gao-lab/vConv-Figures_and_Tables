...
def recover_ker(model, mode="vConv"):
    """
    :param resultPath:
    :param modeltype:
    :param input_shape:
...
def mkdir(path):
    ...
    if not isExists:
        ...
        return False
def GenerateMotif(model,seq_pos_matrix, OutputDir, mode="vConv"):
    """
...
def Conv(data_dir, out_dir):
    ...
    # seq = f["seq"].value
    seq = read_and_decode(data_dir)
    GenerateMotif(seqnn_model.model, seq, out_dir, mode="Conv")
...
if __name__ == '__main__':
    ...
    vConv(Tfrpath, out_dir)

    out_dir = "../../../output/result/BassetCompare/basset/output/PWM/"
    Conv(Tfrpath, out_dir)
