...
def conv_block_vCNN(inputs, filters=None, kernel_size=1, activation='relu', activation_end=None,
               strides=1, dilation_rate=1, l2_scale=0, dropout=0, conv_type='standard', residual=False,
               pool_size=1, batch_norm=False, bn_momentum=0.99, bn_gamma=None, bn_type='standard',
               kernel_initializer='he_normal', padding='same'):
  """Construct a single convolution block.
  ...
  current = layers.activate(current, activation)
  # convolution
  kernel_size = min(current.shape[1], kernel_size*2)
  current = conv_layer(filters=filters,kernel_size=kernel_size,strides=strides,
                       padding=padding,use_bias=False,kernel_initializer=kernel_initializer,
                       kernel_regularizer=tf.keras.regularizers.l2(l2_scale))(current)
...
def conv_block_2d_vCNN(inputs, filters=128, activation='relu', conv_type='standard',
    kernel_size=1, strides=1, dilation_rate=1, l2_scale=0, dropout=0, pool_size=1,
    batch_norm=False, bn_momentum=0.99, bn_gamma='ones', bn_type='standard', symmetric=False):
  """Construct a single 2D convolution block.   """
  ...
  current =vCNNLayer(
    input_shape=input_shape, kernel_size=kernel_size, filters=filters,
...
def VCNNconv_tower(inputs, filters_init, filters_mult=1, repeat=1, **kwargs):
  """Construct a reducing convolution block.
  ...
  for ri in range(repeat):
    # convolution
    current = conv_block_vCNN(current,
      filters=int(np.round(rep_filters)),
      **kwargs)
...
def dilated_residual_2d(inputs, filters, kernel_size=3, rate_mult=2,
    dropout=0, repeat=1, symmetric=True,vCNN=True, **kwargs):
  """Construct a residual dilated convolution block.
  """
  ...
  for ri in range(repeat):
    ...
    # dilate
    if vCNN:
      current = conv_block_2d_vCNN(current,
        filters=filters,
        kernel_size=kernel_size,
        dilation_rate=int(np.round(dilation_rate)),
        bn_gamma='ones',
