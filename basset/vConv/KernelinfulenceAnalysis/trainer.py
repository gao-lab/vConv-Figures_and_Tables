...
class Trainer:
    ...
    if loss==None:
      ...
      for model in seqnn_model.models:
        ...
        model.compile(loss=loss,
                      ...
                      metrics=model_metrics)
    self.compiled = True
  def SetVCNNParameter(self,seqnn_model):
    """
    ...
    for layer in seqnn_model.model.layers:
      if "v_cnn_layer" in layer.name:
        ...
        kernelen = param[0].shape[0]
        if i !=8:
          init_len = int(kernelen/2)
        else:
          init_len = kernelen
        init_num = k_weights_shape[2]
        init_part_left = np.zeros([1, k_weights_shape[1], init_num]) + (kernelen - init_len) / 2
        init_part_right = np.zeros((1, k_weights_shape[1], init_num)) + (kernelen + init_len) / 2
  ...
  def fit_keras(self, seqnn_model, vCNN=None):
    ...
    # seqnn_model.model.layers[4].get_weights()[2]
    if vCNN:
      seqnn_model=self.SetVCNNParameter(seqnn_model)
    if self.loss == 'bce':
