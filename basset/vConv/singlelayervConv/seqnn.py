...
class CustomModel(keras.Model):
  def train_step(self, data):
    ...
    gradients = tape.gradient(loss, trainable_vars,unconnected_gradients='zero')
    # Update weights
...
class SeqNN():
  ...
  def build_model(self, save_reprs=False):
    ...
    self.model_trunk = CustomModel(inputs=sequence, outputs=trunk_output)
    ###################################################
    # heads
    ###################################################
    ...
    for ho in self.head_output:
        self.models.append(CustomModel(inputs=sequence, outputs=ho))
