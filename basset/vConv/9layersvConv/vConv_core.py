...
class vCNNLayer(Conv1D):
    ...
    def build(self, input_shape):
        ...
        for i in range(input_dim - 1):
            tem.append(0.3 / (input_dim - 1))

        self.threshold = np.sum(-np.log(tem) / (np.log(2)) * tem)
    ...
    def call(self, inputs):
        def DingYTransForm(KernelWeights):
            ...
            ExpTensor = K.repeat_elements(ExpArray, ExpArrayT.shape[1], axis=1)
        ...
        if self.rank == 1:
            ...
            ShanoylossValue = K.sum((ShanoyE * MaskValue - K.cast(self.threshold, dtype='float32'))
                                    * (ShanoyE * MaskValue - K.cast(self.threshold, dtype='float32'))
                                    )
